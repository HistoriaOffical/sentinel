...
def is_valid_historia_address(address, network='mainnet'):
    ...
    historia_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Historia address
        return False

    if (address_version != historia_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.historiad_serialise())
...
def SHIM_serialise_for_historiad(sentinel_hex):
    ...
    # superblock => "trigger" in historiad
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # historiad expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    historiad_hex = serialise(obj)
    return historiad_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('historia.conf')
