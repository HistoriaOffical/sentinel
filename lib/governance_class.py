...
class GovernanceClass(object):
    ...
    def vote(self, historiad, signal, outcome):
        return self.go.vote(historiad, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, historiad):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(historiad, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(historiad, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import historialib
        obj_data = historialib.SHIM_serialise_for_historiad(self.serialise())
    ...
    def historiad_serialise(self):
        import historialib
        return historialib.SHIM_serialise_for_historiad(self.serialise())
