...
class GovernanceObject(BaseModel):
    ...
    # sync historiad gobject list with our local relational DB backend
    @classmethod
    def sync(self, historiad):
        golist = historiad.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_historiad(historiad, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_historiad(self, historiad, rec):
        import decimal
        import historialib
        import binascii
        import gobject_json
        ...
        # get/create, then sync payment amounts, etc. from historiad - Historiad is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(historiad)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from historiad! %s" % e)
            govobj.vote_delete(historiad)
            return (govobj, None)
    ...
    def vote_delete(self, historiad):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(historiad, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, historiad, signal, outcome):
        import historialib
        ...
        output = historiad.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = historialib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(historiad, signal)

    def sync_network_vote(self, historiad, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = historiad.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (historialib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = HISTORIAD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not historialib.is_valid_historia_address(addr, config.network):
    ...
    def hash(self):
        import historialib
        return historialib.hashit(self.serialise())
