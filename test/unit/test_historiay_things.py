...
def test_valid_historia_address():
    from historialib import is_valid_historia_address

    main = valid_historia_address()
    test = valid_historia_address('testnet')

    assert is_valid_historia_address(main) is True
    assert is_valid_historia_address(main, 'mainnet') is True
    assert is_valid_historia_address(main, 'testnet') is False

    assert is_valid_historia_address(test) is False
    assert is_valid_historia_address(test, 'mainnet') is False
    assert is_valid_historia_address(test, 'testnet') is True


def test_invalid_historia_address():
    from historialib import is_valid_historia_address

    main = invalid_historia_address()
    test = invalid_historia_address('testnet')

    assert is_valid_historia_address(main) is False
    assert is_valid_historia_address(main, 'mainnet') is False
    assert is_valid_historia_address(main, 'testnet') is False

    assert is_valid_historia_address(test) is False
    assert is_valid_historia_address(test, 'mainnet') is False
    assert is_valid_historia_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from historialib import elect_mn
...
def test_hash_function():
    ...
    hex_hash = "%x" % historialib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(historialib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(historialib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(historialib.blocks_to_seconds(16616)) == 2612035
